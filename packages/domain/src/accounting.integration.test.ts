...
describe('Fase 8b — domínio contabilístico (integração)', () => {
  ...
  it('#25 seed e dados da 8a (demo-company) continuam intactos', async () => {
    ...
    expect(contas).toBe(47); // 37 base (8a) + 114/115 (8c.1) + 312 (S8) + 42/421/55/551 (inventário, S9) + 422 (S10b) + 24/241 (adiantamentos, S17)
    expect(periodos).toBe(12);
    expect(mappings).toBe(20); // 15 (8a) + OPENING_BALANCE_EQUITY (S8) + INVENTORY_SURPLUS/INVENTORY_SHORTAGE (S9) + OTHER_INCOME (S10b) + CUSTOMER_ADVANCES (S17)
