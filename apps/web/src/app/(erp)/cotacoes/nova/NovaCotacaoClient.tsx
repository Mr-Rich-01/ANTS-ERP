...
import { createQuotationAction } from '@/app/(erp)/cotacoes/actions';
...
export function NovaCotacaoClient({ customers, products, canDiscount }: { customers: CustomerOpt[]; products: ProductOpt[]; canDiscount: boolean }) {
  const router = useRouter();
  const [pending, startTransition] = useTransition();
  const [idempotencyKey] = useState(() => createIdempotencyKey());
  const [issueDate] = useState(() => civilDateInTimeZone());
  const [validUntil, setValidUntil] = useState(() => plusDays(civilDateInTimeZone(), 15));
  ...
  const productDefaults = useMemo<ComboOption[]>(
    () => products.map((p) => ({ value: p.id, label: p.name, sublabel: `${p.sku} · ${fmt(p.price)}`, data: { sku: p.sku, name: p.name, price: p.price } })),
    [products],
  );
  ...
  const addProduct = (option: ComboOption | null) => {
    ...
    setLines((L) => {
      ...
      return [...L, { productId: option.value, name: String(d.name ?? option.label), sku: String(d.sku ?? ''), price: Number(d.price ?? 0), qty: 1, disc: 0 }];
    });
  };
  ...
  const canSubmit = Boolean(customerId) && Boolean(validUntil) && lines.length > 0 && !pending;

  const emit = () => {
    setError(null);
    if (!customerId) return setError('Seleccione um cliente.');
    if (!validUntil) return setError('Seleccione a validade da cotação.');
    ...
    startTransition(async () => {
      ...
      else if (res.id) router.push(`/cotacoes/documento?id=${res.id}`);
    });
  };
  ...
  return (
    <div style={{ padding: '14px 26px 30px', display: 'grid', gridTemplateColumns: '1fr 340px', gap: 16, alignItems: 'start' }}>
      <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
        ...
        <div style={cardBox}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 13 }}>
            ...
            <div>
              ...
              <input type="date" required disabled value={issueDate} style={{ ...selectStyle, background: 'var(--card2)', cursor: 'default' }} />
            </div>
            <div>
              <label style={label}>Válida até</label>
              <input type="date" required min={issueDate} value={validUntil} onChange={(e) => setValidUntil(e.target.value)} style={selectStyle} />
            </div>
            <div>
                ...
                {lines.length === 0 ? (
                  <tr>
                    <td colSpan={6} style={{ padding: '26px 14px', textAlign: 'center', color: 'var(--text3)', fontSize: 13 }}>
                      Adicione produtos à cotação. Sem reserva nem saída de stock.
                    </td>
                  </tr>
                ) : (
                  lines.map((l) => {
                      ...
                      <tr key={l.productId} style={{ borderTop: '1px solid var(--bd-soft2)' }}>
                        ...
                        <td style={{ padding: '11px 10px' }}>
                          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 5 }}>
                            ...
                            <span className="tnum" style={{ minWidth: 26, textAlign: 'center', fontSize: 13, fontWeight: 600, color: 'var(--text)' }}>{l.qty}</span>
                            <button onClick={() => setQty(l.productId, 1)} style={qtyBtn}>
                              <Icon name="plus" size={12} />
                            </button>
                          </div>
                        </td>
                        <td className="tnum" style={{ padding: '11px 14px', textAlign: 'right', fontSize: 13, color: 'var(--text)', whiteSpace: 'nowrap' }}>{fmt(l.price)}</td>
                        ...
                        <td className="tnum" style={{ padding: '11px 14px', textAlign: 'right', fontSize: 13, fontWeight: 700, color: 'var(--text)', whiteSpace: 'nowrap' }}>{fmt(lineTotal)}</td>
        ...
        {/* Observações */}
        <div style={cardBox}>
          <div style={{ fontSize: 14, fontWeight: 700, color: 'var(--text)', marginBottom: 13 }}>Observações</div>
          <textarea value={notes} onChange={(e) => setNotes(e.target.value)} placeholder="Condições comerciais, prazos de entrega, validade dos preços…" style={{ width: '100%', minHeight: 74, resize: 'vertical', border: '1px solid var(--border)', borderRadius: 10, padding: '11px 12px', fontSize: 13, background: 'var(--card)', color: 'var(--text)', outline: 'none' }} />
        </div>
      </div>
      ...
      <div style={{ position: 'sticky', top: 8, display: 'flex', flexDirection: 'column', gap: 14 }}>
        <div style={cardBox}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 14 }}>
            ...
            <span style={{ fontSize: 11, fontWeight: 600, color: 'var(--info)', background: 'var(--info-bg)', padding: '3px 10px', borderRadius: 20 }}>Cotação</span>
          </div>
          ...
          <div style={{ fontSize: 11.5, color: 'var(--text3)', lineHeight: 1.5, marginTop: 10 }}>
            A cotação não movimenta stock, saldos nem contabilidade.
          </div>
          ...
          <button onClick={emit} disabled={!canSubmit} style={{ width: '100%', height: 46, marginTop: 14, borderRadius: 11, border: 'none', background: ACCENT, color: '#fff', fontSize: 14, fontWeight: 700, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, opacity: canSubmit ? 1 : 0.6, cursor: canSubmit ? 'pointer' : 'default' }}>
            ...
            {pending ? 'A emitir…' : 'Emitir cotação'}
          </button>
          <button onClick={() => router.push('/cotacoes')} style={{ width: '100%', height: 42, marginTop: 8, borderRadius: 11, border: '1px solid var(--border)', background: 'var(--card)', color: 'var(--text2)', fontSize: 13, fontWeight: 600, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, cursor: 'pointer' }}>
            <Icon name="x" size={16} />
            Cancelar
          </button>
        </div>
