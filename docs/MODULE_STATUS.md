...
| Autenticação | Login, sessões (Auth.js), troca de password inicial, logout, protecção de rotas | Parcial | 55 | login e2e ✓ (HTTP), domain 6 testes | Recuperação de password, política/expiração, sessões na BD | Fundação | 2026-06-26 |
| Multiempresa / RBAC | RequestContext, menu por permissões, guard de domínio; Admin (utilizadores/perfis/auditoria/empresa) com dados reais | Parcial | 45 | requirePermission testado, /admin e2e ✓ | CRUD (criar/editar utilizadores e perfis), sessões persistidas, isolamento Prisma automático, auditoria nas mutações | Autenticação | 2026-06-26 |
