...
_Última actualização: 2026-07-22 (S17)_

Estado vivo do projecto. O conhecimento permanente (arquitectura, regras, comandos) está
em [`CLAUDE.md`](CLAUDE.md).

**Último commit funcional:** pendente na branch `s17-notas-devolucoes` (Sessão S17 — backlog do cliente, Prioridade 3; a S16 foi integrada na `main` por fast-forward em 2026-07-22)
**Fase concluída:** `S17 — Notas, Adiantamentos e Devoluções` (Recibo de Adiantamento série `RA` com passivo 241 «Adiantamentos de clientes» — mapping novo `CUSTOMER_ADVANCES` no grupo 24 «Outros credores»; aplicação de RA a facturas gera REC com método novo «Adiantamento» sem tesouraria nova; Devolução ao Cliente série `DEV` com origens RA/NC; a NC continua a ser a única fonte de reversão de venda/stock; NC sobre VD verificada sem alterações — fases anteriores: S16 Relatório de Vendas; **S12 Produção ⏸ EM ESPERA** a aguardar respostas KOKO, trabalho parqueado na branch `s12-producao`)
...
**Próximo passo:** `Sessão S18 — Stock e Contabilidade` (backlog do cliente, Prioridade 4: folha de contagem física imprimível, balancete com filtro por classe e contas com/sem movimento, Razão Geral «todas as contas», impressão + Excel destas listagens reutilizando o `exportTableToXlsx` da S16) — não iniciar sem instrução explícita, em branch própria. Pendências: **IVA em adiantamentos** (registada na S17 — validar com o contabilista do cliente; hoje o RA não liquida IVA), anulação de RA e de REC de adiantamento aplicado (fora da V1 — o `reverseCustomerPayment` bloqueia RECs com método `ADVANCE` com mensagem clara), adopção incremental do `exportTableToXlsx` nas restantes tabelas e filtro por loja/filial no relatório de vendas. A `S12 — Produção` fica ⏸ EM ESPERA (respostas KOKO pendentes; docs e catálogo demo parqueados na branch `s12-producao`, commit 5698277); mantém-se pendente o smoke manual final em browser externo/limpo (logout, clique final POS, Fecho de Caixa V1) antes da demo externa
...
multiempresa 7/7 · **POS 12/12** (`pnpm test:integration:pos`) · **relatorios 24/24** (`pnpm test:integration:reports`) · **relatório de vendas S16 15/15** (`pnpm test:integration:reports:sales`, inclui reabertura do XLSX com exceljs) · **documentos de venda S15 12/12** (`pnpm test:integration:invoices:vd`) · **Fecho de Caixa V1 11/11** (`pnpm test:integration:treasury:cash-closing`) · `pnpm build` OK em Windows nativo (49 rotas, incluindo `/relatorios/vendas` + `/relatorios/vendas/exportar`, `/api/health`, `/relatorios/exportar`, `/contabilidade/exportar` e `/tesouraria/fecho/exportar`) ·
