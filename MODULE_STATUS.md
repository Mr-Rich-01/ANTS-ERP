...
_Última actualização: 2026-07-22 (S16)_

Estado vivo do projecto. O conhecimento permanente (arquitectura, regras, comandos) está
em [`CLAUDE.md`](CLAUDE.md).

**Último commit funcional:** pendente na branch `s16-relatorio-vendas` (Sessão S16 — backlog do cliente, Prioridade 2; a S15 foi integrada na `main` por fast-forward em 2026-07-22)
**Fase concluída:** `S16 — Relatório de Vendas + Exportação Excel` (página `/relatorios/vendas` conforme o modelo do cliente — grupos VD/Facturas, sub-totais e TOTAL GERAL só de activos, IVA real por documento; exportação XLSX server-side com exceljs e helper genérico `exportTableToXlsx` para as restantes tabelas — fases anteriores: S15 Documentos de Venda; **S12 Produção ⏸ EM ESPERA** a aguardar respostas KOKO, trabalho parqueado na branch `s12-producao`)
...
**Próximo passo:** `Sessão S17 — Notas, Adiantamentos e Devoluções` (backlog do cliente, Prioridade 3: NC sobre VD, Recibo de Adiantamento, Devolução ao Cliente — 🔒 mapa D/C do adiantamento e da devolução a aprovar antes de código) — não iniciar sem instrução explícita, em branch própria. Pendências registadas na S16: adopção incremental do `exportTableToXlsx` nas restantes tabelas (a S18 reutiliza) e filtro por loja/filial no relatório de vendas (`Invoice.branchId` só é preenchido quando a sessão tem filial activa). A `S12 — Produção` fica ⏸ EM ESPERA (respostas KOKO pendentes; docs e catálogo demo parqueados na branch `s12-producao`, commit 5698277); mantém-se pendente o smoke manual final em browser externo/limpo (logout, clique final POS, Fecho de Caixa V1) antes da demo externa
...
| S17 | Notas, Adiantamentos e Devoluções (backlog cliente P3) | 🗺️ próximo |
