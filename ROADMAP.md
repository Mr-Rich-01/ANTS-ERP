...
- **S10b — Anulação de NC + ND→Outros proveitos** *(pendente: schema da anulação — `credit_notes.cancelledAt/ById/Reason`, `stock_movements.creditNoteId`, scope `CREDIT_NOTE_CANCEL` —, `cancelCreditNote` com estorno simétrico dos DOIS eventos da NC + reversão dos movimentos IN da devolução, desbloqueio verificado do cancelamento de facturas com NC anulada — o guard já filtra por `ISSUED` —, conta 422 «Outros proveitos operacionais» + mapping `OTHER_INCOME` no seed e no `createDebitNote`. Decisão aprovada: a ND histórica em 411 — ND 2026/0001, base 150,00 MT — fica documentada, não se reclassifica.)*
- **S10c — Lançamentos manuais + retroactivo** *(pendente: UI dos lançamentos manuais — o domínio 8b já está completo com `accounting.prepare/post/reverse` —; operação genérica e reutilizável de regularização de existências — gate próprio, idempotente, auditada, valor calculado como stock físico ao avgCost − saldo 131 no momento da execução com detalhe por produto para revisão e confirmação — usada primeiro na demo, onde a divergência era −311 411,00 MT em 2026-07-18 dominada pelo stock do seed da Fase 4 sem abertura.)*
